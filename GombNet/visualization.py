...
def plot_case_summaries(
    ...
    for case_name in case_order:
        ...
        plot_generalization_summary(
            ...
            title=f"Blob-Net Summary: {case_name.title()}",
        )
...
def plot_loss_curves(model_names: Sequence[str], output_dir: Path) -> Path | None:
    ...
    fig.suptitle("Blob-Net Training Curves", fontsize=14)
...
def save_real_image_overview(
    ...
    fig, axes = plt.subplots(len(variant_names), 3, figsize=(14, 4.8 * len(variant_names)), constrained_layout=True)
    if len(variant_names) == 1:
        axes = np.array([axes])

    for row_index, variant_name in enumerate(variant_names):
        image = image_variants[variant_name]
        heatmap = heatmaps[variant_name]
        coords = coordinates[variant_name]

        axes[row_index, 0].imshow(image, cmap="gray")
        axes[row_index, 0].set_title(f"{variant_name}\nnormalized input")
        axes[row_index, 0].axis("off")

        axes[row_index, 1].imshow(heatmap, cmap="magma")
        axes[row_index, 1].set_title(f"{variant_name}\npredicted heatmap, threshold={threshold_rel:.2f}")
        axes[row_index, 1].axis("off")

        axes[row_index, 2].imshow(image, cmap="gray")
        if len(coords):
            axes[row_index, 2].scatter(coords[:, 1], coords[:, 0], s=10, c="cyan", alpha=0.65, linewidths=0.25)
        title = f"{variant_name}\noverlay, detections={len(coords)}"
        if pixel_size_angstrom is not None:
            title += f", {pixel_size_angstrom:.3f} A/px"
        axes[row_index, 2].set_title(title)
...
def save_real_crop_gallery(
    ...
    fig, axes = plt.subplots(len(crop_boxes), 3, figsize=(13, 4.2 * len(crop_boxes)), constrained_layout=True)
    if len(crop_boxes) == 1:
        axes = np.array([axes])

    for row_index, (y0, y1, x0, x1) in enumerate(crop_boxes):
        crop_image = image[y0:y1, x0:x1]
        crop_heatmap = heatmap[y0:y1, x0:x1]
        mask = (
            (coordinates[:, 0] >= y0)
            & (coordinates[:, 0] < y1)
            & (coordinates[:, 1] >= x0)
        ...
        crop_coords = coordinates[mask].copy()
        crop_coords[:, 0] -= y0
        crop_coords[:, 1] -= x0

        axes[row_index, 0].imshow(crop_image, cmap="gray")
        axes[row_index, 0].set_title(f"Crop {row_index + 1}: input")
        axes[row_index, 0].axis("off")

        axes[row_index, 1].imshow(crop_heatmap, cmap="magma")
        axes[row_index, 1].set_title(f"Crop {row_index + 1}: predicted heatmap")
        axes[row_index, 1].axis("off")

        axes[row_index, 2].imshow(crop_image, cmap="gray")
        if len(crop_coords):
            axes[row_index, 2].scatter(crop_coords[:, 1], crop_coords[:, 0], s=14, c="cyan", alpha=0.75, linewidths=0.35)
        axes[row_index, 2].set_title(f"Crop {row_index + 1}: overlay, detections={len(crop_coords)}")
        axes[row_index, 2].axis("off")

    fig.suptitle(f"Real image crops: {variant_name}", fontsize=14)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return output_path
